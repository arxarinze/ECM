...
class ManageDocument
{
    ...
    public function edit($c, $e)
    {
        ...
        if($res === true)
        {
        $zip->extractTo($this->tmppath.'/'.$this->Document_Name);
        $zip->close(); 
        }
        $file = file_get_contents('tmp/document/'.$this->Document_Name.'/word/document.xml');
        for ($i=0; $i<count($c); $i++)
        {
            $file = str_replace($c[$i], $e[$i], $file);
        }
        file_put_contents('tmp/document/'.$this->Document_Name.'/word/document.xml', $file);
        $rootPath = realpath('tmp\\document\\'.$this->Document_Name.'\\');
$zip->open('documents\\'.$this->Document_Name.'.zip', ZipArchive::CREATE | ZipArchive::OVERWRITE);
...
$zip->close();
         ...
         rename('documents/'.$this->Document_Name.'.zip', 'documents/'.$this->Document_Name.'.'.$this->Document_Extension);
    }
}
?>