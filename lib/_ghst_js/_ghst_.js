...
this.ajaxGetData = function (data)
{
	var XMLHttpR = new XMLHttpRequest();
	var obj = this.convert_constructor();
	if(XMLHttpR)
	{
		XMLHttpR.open("POST", data);


		XMLHttpR.onreadystatechange = function()
		{
			if (XMLHttpR.readyState == 4 && XMLHttpR.status == 200) {
				object.innerHTML = XMLHttpR.responseText;
			}
		}
		XMLHttpR.send(null);
	}
...
this.ajaxRealTime = function (dataSource)
{
...
{	
	...
	var stylePositionText = document.createTextNode(".tooltip{ padding-right:10px; padding-left: 10px; padding-top: 10px;background-color:lightyellow; display: none; color: gray; overflow: hidden; width:auto; height: 30px; position: absolute;box-shadow: 0px 0px 0px; border-radius: 10px 10px 10px; z-index: 7999;}");
	...
	div.innerHTML = text;
	obj[i].addEventListener('mouseover' && 'mousemove', function (event){document.body.appendChild(div); div.style.display = 'block'; var po = cMP(event); div.style.opacity=1; div.style.zIndex=7999; div.style.top = po.y+"px"; div.style.left = (30+po.x)+"px";}, true);
...
this.load= function (fps,type,speed,callback,color)// used in making loaders works with "Ajax only""......cant replace the refresh method of default forms
{
	this.convert_constructor();
		...
		div.setAttribute('style','position: fixed; height:100%; width: 100%;background-color:'+color+';top: 0px; left:-1887px; border-right: 0px solid white; z-index: 9000');
		...
		var div = document.createElement('div');
		div.setAttribute('id', 'cover');
		div.setAttribute('style', 'position: fixed; height:100%; width: 100%; background-color:'+color+';top: 0px; left:-9999px; border-right: 0px solid white;z-index:9000');
		document.body.appendChild(div);
		div.style.height = window.outerHeight+"px";
		div.style.left = (-body_width)+"px";
		var x = div.style.left;
		...
		div2.setAttribute('style', 'position: fixed; height:100%; width: 100%; background-color:'+color+';top: 0px; right:1887px; border-right: 0px solid white;z-index:9000');
		...
		var div = document.createElement('div');
		div.setAttribute('id', 'cover');
		div.setAttribute('style', 'position: fixed; height:100%; width: 100%; background-color:'+color+';top: 0px; left:0px; border-right: 0px solid white; z-index: 9000');
		...
		var div2 = document.createElement('div');
		div2.setAttribute('id', 'cover1');
		div2.setAttribute('style', 'position: fixed; height:100%; width: 100%; background-color:'+color+'; bottom: 0px; right:0px; border-right: 0px solid white; z-index: 9000');
		document.body.appendChild(div);
		document.body.appendChild(div2);
		div2.style.height = (window.innerHeight)+'px';
		div2.style.top = (-body_height)+"px";
